...
export default function AdminNavbar({ title }) {
    ...
    const onLogout = () => {
        localStorage.removeItem('movie-booking:admin:google');
            ...
            <Flex
                ...
                <Flex flex={{ base: 1 }} justify={{ base: 'center', md: 'start' }}>
                    <Text
                        ...
                        fontWeight={'bold'}
                        onClick={() => navigate('/admin/feed')}>  {title}   </Text>
            ...
            {NAV_ITEMS.map((navItem) => (
                <Box key={navItem.label}>
                    <Popover trigger={'hover'} placement={'bottom-start'}>
                        <PopoverTrigger>
                            <RouteLink to={navItem.path} p={2} cursor={'pointer'}>
                                <Text color={navItem.path === location.pathname ? 'black' : 'gray.300'}>{navItem.label}</Text>
                            </RouteLink>
                        </PopoverTrigger>
                    </Popover>
                </Box>
...
const NAV_ITEMS = [
    {
        label: 'Monetization and Earnings',
